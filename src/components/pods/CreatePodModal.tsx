...
import { X, MessagesSquare, Handshake, Briefcase } from "lucide-react";
import { PodKind } from "@/types/pod";
import { createPod, CURRENT_USER } from "@/lib/mock-pods";

const KIND_OPTIONS: { kind: PodKind; icon: typeof MessagesSquare; title: string; description: string; adds: string; disabled?: boolean }[] = [
  {
    kind: "community",
    icon: MessagesSquare,
    title: "Community POD",
...
export default function CreatePodModal({ onClose, onCreated }: { onClose: () => void; onCreated: (podId: string) => void }) {
  ...
  const [kind, setKind] = useState<PodKind>("community");
                  ...
                  <button
                    ...
                    className={`relative text-left p-4 rounded-lg border-2 transition-all ${
                      opt.disabled
                        ...
                        : active
                        ? "border-[#0066f3] bg-[#0066f3]/5 shadow-sm"
                        : "border-slate-200 hover:border-slate-300"
                    }`}
                  >
                    {opt.disabled && (
                      <span className="absolute top-3 right-3 text-[9.5px] font-bold uppercase tracking-wide text-slate-400 bg-slate-100 px-1.5 py-0.5 rounded-full">
                        Coming soon
                      </span>
                    )}
                    <div className={`w-9 h-9 rounded-lg flex items-center justify-center mb-3 ${active && !opt.disabled ? "bg-[#0066f3] text-white" : "bg-slate-100 text-slate-500"}`}>
                      <Icon size={16} />
                    </div>
          ...
          <>
            <div className="px-6 pt-5 space-y-4">
              <div className="flex items-center gap-2">
                <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-[#0066f3]/10 text-[#0052c2] text-[11px] font-semibold border border-[#0066f3]/25">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-3.5 py-2.5 rounded-lg border border-slate-200 text-[13.5px] bg-white focus:outline-none focus:ring-2 focus:ring-[#0066f3]/30 focus:border-[#0066f3]"
                  >
