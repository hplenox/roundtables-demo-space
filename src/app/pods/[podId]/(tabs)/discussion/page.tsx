      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-4 shadow-sm">
        ...
        {showPoll && (
          <div className="mb-3 p-3 rounded-xl bg-slate-50 border border-slate-200 space-y-2">
            <input
              ...
              className="w-full px-3 py-2 rounded-lg border border-slate-200 text-[12.5px] focus:outline-none focus:ring-2 focus:ring-[#00b8a9]/30"
            />
            ...
            <div className="flex justify-end gap-2 pt-1">
              ...
              <button onClick={handlePostPoll} className="text-[12px] font-semibold text-white bg-[#00b8a9] hover:bg-[#00a89a] px-3 py-1.5 rounded-lg transition-colors">Post poll</button>
            </div>
          </div>
        )}
        ...
        <div className="flex items-center justify-between pt-2 border-t border-slate-100">
          ...
          <button
            onClick={handlePost}
            disabled={!body.trim()}
            className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-lg bg-[#00b8a9] text-white text-[12.5px] font-semibold hover:bg-[#00a89a] disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
          >
            <Send size={13} /> Post
          </button>
        </div>
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="flex items-center gap-4 mt-3">
            <button
              ...
              className={`inline-flex items-center gap-1.5 text-[12px] font-medium transition-colors ${post.liked ? "text-[#00897b]" : "text-slate-400 hover:text-slate-600"}`}
            >
          ...
          {showReply && (
            <div className="flex items-center gap-2 mt-3">
              <input
                ...
                className="flex-1 px-3 py-1.5 rounded-lg border border-slate-200 text-[12.5px] focus:outline-none focus:ring-2 focus:ring-[#00b8a9]/30"
                onKeyDown={(e) => e.key === "Enter" && submitReply()}
              />
              <button onClick={submitReply} className="text-[12px] font-semibold text-white bg-[#00b8a9] hover:bg-[#00a89a] px-3 py-1.5 rounded-lg transition-colors">Send</button>
            </div>
          )}
        </div>
      </div>
  ...
  return (
    <div className="mt-3 rounded-xl border border-slate-200 overflow-hidden">
      <div className="flex items-center justify-between px-3.5 py-2.5 bg-slate-50 border-b border-slate-200">
        <p className="text-[12.5px] font-bold text-slate-700 flex items-center gap-1.5"><CalendarPlus size={13} className="text-[#00897b]" /> {poll.question}</p>
      ...
      <div className="divide-y divide-slate-100">
        {poll.options.map((opt) => {
          ...
          return (
            ...
            >
              <div className="absolute inset-y-0 left-0 bg-[#00b8a9]/10" style={{ width: `${pct}%` }} />
              <span className="relative flex items-center gap-2 text-[12.5px] text-slate-700 font-medium">
                ...
                {mine && <span className="text-[10px] font-bold text-[#00897b]">(your vote)</span>}
              </span>
      ...
      <div className="flex items-center gap-3 px-3.5 py-2.5 bg-white">
        {poll.createdEventId ? (
          ...
          <button onClick={onCreateEvent} className="inline-flex items-center gap-1.5 text-[12px] font-semibold text-white bg-[#00b8a9] hover:bg-[#00a89a] px-3 py-1.5 rounded-lg transition-colors">
